...
"""
This script will scan MEBT BNCH01 and BNCH02 amplitudes. It will track the bunch
to the end of MEBT and caculate the bunch length as a function of 
BNCH01 and BNCH02 amplitudes.
...
bnch01 = accLattice.getNodeForName("LI_MEBT1:BNCH01:Rg01")
bnch01_ind = accLattice.getNodeIndex(bnch01)
...
#real tracking of the bunch. Now we have the bunch at the BNCH01 entrance
accLattice.trackBunch(bunch_in,None,None,0,bnch01_ind-1)
...
print "========== Bunch RMS sizes at BNCH01 entrance:"
...
#==== Now we track the bunch from BNCH01 etrance to the end of the MEBT
bunch = Bunch()
bunch_in.copyBunchTo(bunch)
accLattice.trackBunch(bunch,None,None,bnch01_ind)
...
#==== Now we repeat tracking bunch from BNCH01 to the end of the MEBT
#==== for different values of the E0TL for  BNCH01 and  BNCH02

E0TL_1_arr = []
E0TL_1_arr .append(0.130/1000.)
E0TL_1_arr .append(0.140/1000.)
E0TL_1_arr .append(0.150/1000.)
...
for ind in range(n_points):
	E0TL_2 = E0TL_max*(1.0*(ind+1))/n_points
	if(E0TL_2 > E0TL_min):
		for E0TL_1 in E0TL_1_arr:
			E0TL_1_2_arr.append([E0TL_1,E0TL_2])

n_points = len(E0TL_1_2_arr)	
...
count = 0
for [E0TL_1,E0TL_2] in E0TL_1_2_arr:
	trMtrx = Matrix(7,7)
	bnch01.setParam("E0TL",E0TL_1)
	bnch02.setParam("E0TL",E0TL_2)
	bunch = Bunch()
	bunch_in.copyBunchTo(bunch)
	accLattice.trackBunch(bunch,None,None,bnch01_ind)
	...
	res_sizes_trMtrx_arr.append([E0TL_1,E0TL_2,(x_rms,y_rms,z_rms_deg),mz11,mz12])
	print "E0TL 1,2[keV] =  %5.1f  %5.1f (Sx,Sy,Sz) =   %5.3f  %5.3f   %5.1f  det(TrMtrxZ) = %5.4f "%(E0TL_1*1.0e+6,E0TL_2*1.0e+6,x_rms,y_rms,z_rms_deg,det_z)
	#-------------------
	count += 1
...
line_ind = 0
for [E0TL_1,E0TL_2,(x_rms,y_rms,z_rms_deg),mz11,mz12] in res_sizes_trMtrx_arr:
